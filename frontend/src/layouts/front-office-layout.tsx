...
export const frontOfficeCategories: NavCategory[] = [
  { label: 'Dashboard', icon: LayoutDashboard, to: '/front-office', end: true, items: [] },
  { label: 'Visitor Log', icon: Users, to: '/front-office/visitors', end: true, items: [] },
  { label: 'Gate Passes', icon: DoorOpen, to: '/front-office/gate-passes', end: true, items: [] },
  { label: 'Correspondence', icon: Mail, to: '/front-office/correspondence', end: true, items: [] },
];
  ...
  return (
    <SidebarProvider>
      <Sidebar collapsible="icon">
        <SidebarHeader className="border-b px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col group-data-[collapsible=icon]:hidden">
              <span className="text-sm font-semibold">Front Office</span>
              <span className="text-xs text-muted-foreground">Reception & Visitors</span>
            </div>
          </div>
        </SidebarHeader>
        ...
        >
          <SidebarMenu>
            {frontOfficeCategories.map((cat) => (
              <CategoryNavItem key={cat.label} cat={cat} />
            ))}
          </SidebarMenu>
          <ModuleSwitcher currentModule="/front-office" />
        </SidebarContent>
            ...
            <SidebarMenuItem>
              <DropdownMenu>
                ...
                <DropdownMenuContent side="top" align="start" className="w-56">
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => navigate('/front-office/profile')}>
      ...
      <SidebarInset>
        <ImpersonationBanner />
        <SiteHeader />
        <SubNav categories={frontOfficeCategories} />

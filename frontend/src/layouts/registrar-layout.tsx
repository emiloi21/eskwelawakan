...
export const registrarCategories: NavCategory[] = [
  { label: 'Dashboard', icon: LayoutDashboard, to: '/registrar', end: true, items: [] },
  {
    label: 'Students',
    icon: Users,
    items: [
      ...
      { to: '/registrar/enrollment', label: 'Enrollment' },
      { to: '/registrar/applicants', label: 'Applications' },
      { to: '/registrar/exam-slots', label: 'Exam Slots' },
      { to: '/registrar/import', label: 'CSV Import' },
    ],
  },
  {
    label: 'Academic',
    icon: FileCheck,
    items: [
      { to: '/registrar/requirements', label: 'Requirements' },
      { to: '/registrar/discounts', label: 'Discounts' },
      { to: '/registrar/year-end', label: 'Year-End Promotion' },
    ],
  },
  { label: 'Reports', icon: FileBarChart, to: '/registrar/reports', end: true, items: [] },
  {
    label: 'Operations',
    icon: Briefcase,
    items: [
      { to: '/registrar/supply-requests', label: 'Supply Requests' },
      { to: '/registrar/inventory-tasks', label: 'Inventory Tasks' },
      { to: '/registrar/clearance', label: 'Clearance' },
    ],
  },
];
  ...
  return (
    <SidebarProvider>
      <Sidebar collapsible="icon">
        <SidebarHeader className="border-b px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col group-data-[collapsible=icon]:hidden">
              <span className="text-sm font-semibold">SVHS Registrar</span>
              <span className="text-xs text-muted-foreground">Student Information</span>
            </div>
          </div>
        </SidebarHeader>
        <SidebarContent
        ...
        >
          <SidebarMenu>
            {registrarCategories.map((cat) => (
              <CategoryNavItem key={cat.label} cat={cat} />
            ))}
          </SidebarMenu>
          <ModuleSwitcher currentModule="/registrar" />
        </SidebarContent>
            ...
            <SidebarMenuItem>
              <DropdownMenu>
                ...
                <DropdownMenuContent side="top" align="start" className="w-56">
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => navigate('/registrar/profile')}>
      ...
      <SidebarInset>
        <ImpersonationBanner />
        <SiteHeader />
        <SubNav categories={registrarCategories} />
        <div className="flex flex-1 flex-col min-h-0">
          <div className="flex flex-1 flex-col gap-4 p-4 md:p-6 min-h-0">
            <Suspense fallback={<div className="flex items-center justify-center flex-1"><span className="text-muted-foreground">Loading...</span></div>}>
              <ModuleGate moduleKey="registrar" moduleLabel="Registrar">
                <Outlet />
              </ModuleGate>
            </Suspense>
          </div>

...
export const teacherCategories: NavCategory[] = [
  { label: 'Dashboard', icon: LayoutDashboard, to: '/teacher', end: true, items: [] },
  {
    label: 'Teaching',
    icon: GraduationCap,
    items: [
      { to: '/teacher/my-classes', label: 'My Classes' },
      { to: '/teacher/advisees', label: 'Advisees' },
      { to: '/teacher/flashcards', label: 'Flashcards' },
    ],
  },
  {
    label: 'Operations',
    ...
    <SidebarProvider>
      <Sidebar collapsible="icon">
        ...
        >
          ...
          <ModuleSwitcher currentModule="/teacher" />
        </SidebarContent>
      ...
      <SidebarInset>
        <ImpersonationBanner />
        <SiteHeader />
        <SubNav categories={teacherCategories} />
        <div className="flex flex-1 flex-col min-h-0">
          <div className="flex flex-1 flex-col gap-4 p-4 md:p-6 min-h-0">
            <Suspense fallback={<div className="flex items-center justify-center flex-1"><span className="text-muted-foreground">Loading...</span></div>}>
              <ModuleGate moduleKey="teacher" moduleLabel="Teacher">
                <Outlet />
              </ModuleGate>
            </Suspense>
          </div>

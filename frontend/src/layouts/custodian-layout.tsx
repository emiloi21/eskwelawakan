...
export const custodianCategories: NavCategory[] = [
  { label: 'Dashboard', icon: LayoutDashboard, to: '/custodian', end: true, items: [] },
  {
    label: 'Property',
    icon: Package,
    items: [
      { to: '/custodian/property', label: 'Fixed Assets' },
    ],
  },
  {
    label: 'Supplies',
    icon: Boxes,
    items: [
      { to: '/custodian/consumables', label: 'Consumables' },
      { to: '/custodian/supply-requests', label: 'Supply Requests' },
    ],
  },
  {
    label: 'Inventory',
    icon: ClipboardList,
    items: [
      { to: '/custodian/inventory', label: 'Year-End Inventory' },
    ],
  },
  {
    label: 'Facilities',
    icon: Building2,
    items: [
      { to: '/custodian/facilities', label: 'Manage Facilities' },
      { to: '/custodian/bookings', label: 'Booking Requests' },
    ],
  },
  { label: 'Settings', icon: Settings, to: '/custodian/profile', end: true, items: [] },
];
  ...
  return (
    <SidebarProvider>
      <Sidebar collapsible="icon">
        <SidebarHeader className="border-b px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col group-data-[collapsible=icon]:hidden">
              <span className="text-sm font-semibold">SVHS Custodian</span>
              <span className="text-xs text-muted-foreground">Property &amp; Facilities</span>
            </div>
          </div>
        </SidebarHeader>
        ...
        >
          <SidebarMenu>
            {custodianCategories.map((cat) => (
              <CategoryNavItem key={cat.label} cat={cat} />
            ))}
          </SidebarMenu>
          <ModuleSwitcher currentModule="/custodian" />
        </SidebarContent>
            ...
            <SidebarMenuItem>
              <DropdownMenu>
                ...
                <DropdownMenuContent side="top" align="start" className="w-56">
                  ...
                  <DropdownMenuSeparator />
                  <DropdownMenuItem onClick={() => navigate('/custodian/profile')}>
      ...
      <SidebarInset>
        <ImpersonationBanner />
        <SiteHeader />
        <SubNav categories={custodianCategories} />
        <div className="flex flex-1 flex-col min-h-0">
          <div className="flex flex-1 flex-col gap-4 p-4 md:p-6 min-h-0">
            <Suspense fallback={<div className="flex items-center justify-center flex-1"><span className="text-muted-foreground">Loading...</span></div>}>
              <ModuleGate moduleKey="custodian" moduleLabel="Custodian">
                <Outlet />
              </ModuleGate>
            </Suspense>
          </div>

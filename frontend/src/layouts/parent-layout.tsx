...
export const parentCategories: NavCategory[] = [
  { label: 'Dashboard', icon: LayoutDashboard, to: '/parent', end: true, items: [] },
  { label: 'Messages', icon: MessageSquare, to: '/parent/messages', end: true, items: [] },
];

function CategoryNavItem({ cat }: { cat: NavCategory }) {
  const { pathname } = useLocation();
  const isActive = cat.end ? pathname === cat.to : pathname.startsWith(cat.to ?? '');
  const linkTo = cat.to || '#';
  const Icon = cat.icon;
    ...
    <SidebarProvider>
      <Sidebar collapsible="icon">
        ...
        >
          ...
          <ModuleSwitcher currentModule="/parent" />
        </SidebarContent>
      ...
      <SidebarInset>
        <ImpersonationBanner />
        <SiteHeader />
        <SubNav categories={parentCategories} />
        <div className="flex flex-1 flex-col min-h-0">
          <div className="flex flex-1 flex-col gap-4 p-4 md:p-6 min-h-0">
            <Suspense fallback={<div className="flex items-center justify-center flex-1"><span className="text-muted-foreground">Loading...</span></div>}>
              <ModuleGate moduleKey="parent" moduleLabel="Parent">
                <Outlet />
              </ModuleGate>
            </Suspense>
          </div>
